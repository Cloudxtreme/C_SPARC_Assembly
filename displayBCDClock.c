...
displayBCDClock( const unsigned char clockBCD[] ) {
  ...
  int i = 0;      // Used as counter in outer for loop
  int j = 0;      // Used as counter in inner for loop
  ...
  unsigned char hours_tmp = clockBCD[HR_INDEX];
  unsigned char mins_tmp = clockBCD[MIN_INDEX];
  unsigned char secs_tmp = clockBCD[SEC_INDEX];
  
  /* Outer loop is for each row of clock, inner loop prints each row */
  for( i; i < ROWS; i++ ) {
    for( j; j < NIB_LEN; j++ ) {
